...
class BoltColumnarCacheSuite extends BoltWholeStageTransformerSuite with AdaptiveSparkPlanHelper {
  ...
  private def checkColumnarTableCache(plan: SparkPlan): Unit = {
    assert(
      ...
      plan)
    assert(
      collect(plan) { case v: BoltColumnarToRowExec => v }.size <= 1,
      plan
    )
  }
    ...
    withTempDir {
      dir =>
        ...
        withSQLConf("spark.sql.parquet.fieldId.read.enabled" -> "true") {
          // Write a table with metadata information that Gluten Bolt backend doesn't support,
    ...
    try {
      ...
      assert(
        find(df.queryExecution.executedPlan) {
          case _: RowToBoltColumnarExec => true
          case _ => false
        }.isEmpty
      )
    } finally {
  ...
  test("no ColumnarToRow for table cache") {
    ...
    withSQLConf(GlutenConfig.COLUMNAR_HASHAGG_ENABLED.key -> "false") {
      try {
        ...
        assert(
          find(df.queryExecution.executedPlan) {
            case BoltColumnarToRowExec(child: SparkPlan) if PlanUtil.isGlutenTableCache(child) =>
              true
            case _ => false
          }.isEmpty
        )
