...
class BoltAggregateFunctionsDefaultSuite extends BoltAggregateFunctionsSuite {
  ...
  test("aggregate on join keys still offloads with aligned hash aggregate transformer") {
    val s =
      """
        |select count(1) from
      ...
      runQueryAndCompare(s) {
        df =>
          ...
          assert(executedPlan.exists {
            case _: RegularHashAggregateExecTransformer => true
            case _: FlushableHashAggregateExecTransformer => true
            case _ => false
          })
      }
    }
