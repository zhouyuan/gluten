...
case class RowToBoltColumnarExec(child: SparkPlan) extends RowToColumnarExecBase(child = child) {
  ...
  override def doExecuteColumnarInternal(): RDD[ColumnarBatch] = {
    ...
    val numBytes = BoltConfig.get.boltPreferredBatchBytes
  ...
  override def doExecuteBroadcast[T](): Broadcast[T] = {
    ...
    BroadcastUtils.sparkToBoltUnsafe(
      sparkContext,
      mode,
      schema,
      relation,
