...
class ClickHouseAdaptiveQueryExecSuite extends AdaptiveQueryExecSuite with GlutenSQLTestsTrait {
  ...
  // TODO: re-enable this test after #12845. findTopLevelSortMergeJoinTransform matches the Velox
  // SortMergeJoinExecTransformer, which ClickHouse never produces, so it collects nothing here and
  // the case fails on CH-3.5. The expected counts are also still unverified on ClickHouse.
